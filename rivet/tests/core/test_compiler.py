...
class TestReferenceResolution:
    ...
    def test_resolver_called_and_sql_resolved_set(self) -> None:
        resolver = MagicMock(spec=ReferenceResolver)
        resolver.resolve_references.return_value = "SELECT id FROM schema.src"
    ...
    def test_resolver_no_change_leaves_sql_resolved_none(self) -> None:
        resolver = MagicMock(spec=ReferenceResolver)
        # Return same SQL → no change
        resolver.resolve_references.side_effect = lambda sql, joint, cat: sql
    ...
    def test_resolver_failure_produces_warning(self) -> None:
        resolver = MagicMock(spec=ReferenceResolver)
        resolver.resolve_references.side_effect = RuntimeError("resolver error")
